...
double CalculateLotSize(double slDistance)
{
   ...
   int posCount = CountOpenPositions();
   if(posCount > 0)
      riskAmount = riskAmount / (posCount + 1);
...
void ManageStopLoss()
{
   ...
   double trailActivate = slWidth * Trail_ActivateRR;
   double trailStep     = slWidth * Trail_StepRR;
   int    digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      ...
      if(posType == POSITION_TYPE_BUY)
      {
         ...
         //--- ブレイクイーブン
         else if(UseBreakeven && profit >= beActivate)
         {
      ...
      else if(posType == POSITION_TYPE_SELL)
      {
         ...
         if(UseTrailing && profit >= trailActivate)
         {
            double trailSL = NormalizeDouble(ask + trailStep, digits);
            if(currentSL == 0 || (trailSL < currentSL && trailSL < openPrice - BE_LockPips))
               trade.PositionModify(ticket, trailSL, currentTP);
         }
         //--- ブレイクイーブン
         else if(UseBreakeven && profit >= beActivate)
